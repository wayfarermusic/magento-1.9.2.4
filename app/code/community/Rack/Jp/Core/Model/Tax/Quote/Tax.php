 ...
 */
class Rack_Jp_Core_Model_Tax_Quote_Tax extends Mage_Tax_Model_Sales_Total_Quote_Tax
{
    /**
    ...
    protected function _totalBaseCalculation(Mage_Sales_Model_Quote_Address $address, $taxRateRequest)
    {
        ...
        if($_edition == Mage::EDITION_COMMUNITY && $_version['minor'] < 9) {
            return parent::_totalBaseCalculation($address, $taxRateRequest);
        } elseif($_edition == Mage::EDITION_ENTERPRISE && $_version['minor'] < 14) {
            return parent::_totalBaseCalculation($address, $taxRateRequest);
        }
        ...
        foreach ($taxGroups as $taxId => $data) {
            ...
            $totalTax = Mage::app()->getStore()->roundPrice(array_sum($data['tax']));
            $baseTotalTax = Mage::app()->getStore()->roundPrice(array_sum($data['base_tax']));
