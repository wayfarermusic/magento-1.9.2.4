 ...
 */
class Rack_Jp_Core_Model_Sales_Order_Creditmemo_Total_Tax extends Mage_Sales_Model_Order_Creditmemo_Total_Tax
{
    /**
     * collect tax for creditmemo
     *
     * @param Mage_Sales_Model_Order_Creditmemo $creditmemo
     * @return $this|Mage_Sales_Model_Order_Creditmemo_Total_Tax
     */
    public function collect(Mage_Sales_Model_Order_Creditmemo $creditmemo)
    {
        ...
        if(Mage::helper('jpcore')->canRemoveDecimal($current)) {
            switch($round) {
                case 'ceil':
                    ...
                    $totalHiddenTax = ceil($totalHiddenTax);
                    $shippingTaxAmount = ceil($shippingTaxAmount);
                    break;
                case 'floor':
                    $totalTax = floor($totalTax);
                    $totalHiddenTax = floor($totalHiddenTax);
                    $shippingTaxAmount = floor($shippingTaxAmount);
                    break;
            }
        }
        ...
        if(Mage::helper('jpcore')->canRemoveDecimal($base)) {
            switch($round) {
                case 'ceil':
                    $baseTotalTax = ceil($baseTotalTax);
                    $baseTotalHiddenTax = ceil($baseTotalHiddenTax);
                    $baseShippingTaxAmount = ceil($baseShippingTaxAmount);
                    break;
                case 'floor':
                    $baseTotalTax = floor($baseTotalTax);
                    $baseTotalHiddenTax = floor($baseTotalHiddenTax);
                    $baseShippingTaxAmount = floor($baseShippingTaxAmount);
                    break;
            }
        }
