 ...
 */
class Rack_Jp_Core_Model_Sales_Quote_Address_Total_Discount extends Mage_Sales_Model_Quote_Address_Total_Discount
{
    /**
     * collect discount for quote address
     *
        ...
        if(Mage::helper('jpcore')->canRemoveDecimal($current)) {
            switch($round) {
                case 'ceil':
                    ...
                    $subtotalWithDiscount = ceil($subtotalWithDiscount);
                    break;
                case 'floor':
                    $totalDiscountAmount = floor($totalDiscountAmount);
                    $subtotalWithDiscount = floor($subtotalWithDiscount);
                    break;
                case 'round':
        ...
        if(Mage::helper('jpcore')->canRemoveDecimal($base)) {
            switch($round) {
                case 'ceil':
                    ...
                    $baseSubtotalWithDiscount = ceil($baseSubtotalWithDiscount);
                    break;
                case 'floor':
                    $baseTotalDiscountAmount = floor($baseTotalDiscountAmount);
                    $baseSubtotalWithDiscount = floor($baseSubtotalWithDiscount);
                    break;
                case 'round':
                    $baseTotalDiscountAmount = round($baseTotalDiscountAmount, 2);
                    $baseSubtotalWithDiscount = round($baseSubtotalWithDiscount, 2);
                    break;
            }
        }
        $address->setDiscountAmount($totalDiscountAmount);
